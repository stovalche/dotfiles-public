...
function prompt_hostname -d "Set current hostname"
  set -l BG 073642
  set -l FG 93a1a1

  set -g HOSTNAME_PROMPT ""
  if [ "$theme_hostname" = "always" -o \( "$theme_hostname" != "never" -a -n "$SSH_CLIENT" \) ]
    set -g HOSTNAME_PROMPT (cat /proc/sys/kernel/hostname)
    if [ "$HOSTNAME_PROMPT" ]
...
function prompt_dir -d "Display the current directory"
  ...
  set -g current_dir_name (basename $(pwd))
  set -g current_dir_head (echo (prompt_pwd) | awk -F "$current_dir_name" '{print $1}')

  if [ "$current_dir_name" = (whoami) ]
    set -g current_dir_head ""
    set -g current_dir_name "~"
  end

  prompt_segment $BG $FG (
    echo -n $current_dir_head
    set_color -i -o $FG
    echo -n $current_dir_name
    set_color normal
    set_color -b $BG
  )
  prompt_segment $BGsep $FGsep ""
  prompt_segment $BGsep1 $FGsep1 ""
  prompt_segment $BGsep2 $FGsep2 ""
end
...
function prompt_git -d "Display the current git state"
  ...
  if command git rev-parse --is-inside-work-tree >/dev/null 2>&1
    ...
    prompt_segment $BG white (echo -ne "\033[1;37m$PROMPT" )
  end
end
...
function fish_right_prompt -d "Show right prompt segment"
end
